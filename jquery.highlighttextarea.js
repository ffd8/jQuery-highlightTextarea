  ...
  $.fn.highlightTextarea = function(options) {
      ...
      this.each(function() {
          ...
          function applyText(text) {
              ...
              //Store variables of the text within the position ranges
              var topositions = "";
			  var highlightWithinPositions = "";
			  var afterHighlight = "";

			  if(options.bockHighlight != "none")
			  {
				   topositions = text.substring(0,options.fromPos);
					highlightWithinPositions = text.substring(options.fromPos, options.toPos);
					afterHighlight = "";
				  if(options.toPos < text.length -1)
				  {
					afterHighlight = text.substring(options.toPos, text.length - 1);
				  }

			  }
				//flor block just highlight based on position, not words
				if(options.bockHighlight == "block")
				{
			  ...
			  else
			  {
				  ...
				  if (options.words[0] != "") {
					...
					for (var i=1;i<options.words.length;i++) replace+= '|'+options.words[i];
					  {
						  //if we are searching in a range use our stored section in place of the full text
						if(options.bockHighlight == "withinrange")
						{
							highlightWithinPositions = replaceAll(highlightWithinPositions, replace, "<span class=\"highlight\" style=\"background-color:"+options.color+";\">$1</span>");
						}
						else
						{
							///default behaviour
						...
						//if we used a range add the end back on afterwards
					  if(options.bockHighlight == "withinrange")
						{
							text = highlightWithinPositions + afterHighlight;
						}
				  }
			  }
              //$textarea.html("");
